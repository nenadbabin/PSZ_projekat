...
def najzastupljeniji_delovi_beograda(data_frame: DataFrame):
    # a)
...
def broj_stanova_po_kvadraturi(data_frame: DataFrame):
    # b)
    ...
    for i in range(0, 5):
        data = data_frame
        maska = (data['kvadratura'] >= leva_granica) & (data['kvadratura'] <= desna_granica) & (
                data['tip_nekretnine'] == 'Stan')
        data = data[maska]
        brojevi_stanova.append(data.shape[0])
        leva_granica += 15
        desna_granica += 15
...
def nekretnine_po_dekadatama(data_frame: DataFrame):
    # c)
...
def odnos_nekratnina_na_podaju_i_za_iznajmljivanje(data_frame: DataFrame):
    # d)
...
def nekretnine_po_opsezima(data_frame: DataFrame):
    # e)
    ...
    for i, a in enumerate(autotexts):
        a.set_text(
            f"{brojevi_nekretnina_na_prodaju[i]} ({round((brojevi_nekretnina_na_prodaju[i] / np.array(brojevi_nekretnina_na_prodaju).sum()) * 100, 2)})%")
...
def nekretnine_sa_parkingom(data_frame: DataFrame):
    # f)
    ...
    for i, a in enumerate(autotexts):
        a.set_text(
            f"{nekretnine_parking[i]} ({round((nekretnine_parking[i] / np.array(nekretnine_parking).sum()) * 100, 2)})%")
